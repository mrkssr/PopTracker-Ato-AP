...
Manual_Ato_Mrks_location = {}
Manual_Ato_Mrks_location.__index = Manual_Ato_Mrks_location
...
function Manual_Ato_Mrks_location.new(name)
    local self = setmetatable({}, Manual_Ato_Mrks_location)
    if name then
...
function Manual_Ato_Mrks_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = Manual_Ato_Mrks_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function Manual_Ato_Mrks_location:connect_two_ways(exit, rule)
...
function Manual_Ato_Mrks_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function Manual_Ato_Mrks_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function Manual_Ato_Mrks_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function Manual_Ato_Mrks_location:accessibility()
...
-- 
function Manual_Ato_Mrks_location:discover(accessibility, keys)
...
entry_point = Manual_Ato_Mrks_location.new("entry_point")
